...
make: *** [dfu-launch] Error -2
```

To confirm that the dfu programming is successful, check for 
...
Ignore the last line `make: *** [dfu-launch] Error -2`, this is not an error. It was produced because the command `dfu-programmer atmega32u2 launch --no-reset` did not return any error code to `make`.
{: .warning }
